...
def list_products() -> str:
    ...
    Returns:
        A formatted list of every product with its ID, name, price, and category
    """
    lines = [
        f"{pid}: {p['name']} - ${p['price']} ({p['category']})"
...
3. list_products() - List every product currently in the catalog

When a customer asks broadly what you sell, what's available, or wants to browse (rather than naming a specific item), call list_products() instead of guessing or inventing example products. Always use the appropriate tool rather than guessing."""
...
def get_or_create_agent():
    ...
    if _agent is None:
        _agent = Agent(
            ...
            tools=[get_return_policy, get_product_info, list_products],
        )
    return _agent
